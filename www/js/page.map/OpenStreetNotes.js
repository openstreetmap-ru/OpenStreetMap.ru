L.OpenStreetNotes = L.FeatureGroup.extend({
	options : {
		...
		showOpen:   true,
		showClosed: true,
		iconOpen:  "/img/Open_note_marker.png",
		iconClosed:"/img/Closed_note_marker.png",
	...
	initialize : function(options)
	{
		...
		putNoteAJAXMarker.layers.push(this);
	...
	loadBugs : function()
	{
		...
		var url = this.makeRequestUrl("bbox="
			+     round(sw.lng, 5)
			+ ","+round(sw.lat, 5)
			+ ","+round(ne.lng, 5)
			+ ","+round(ne.lat, 5));
...
//            alert('Error loading Notes');
           ...
           $(json.features).each(function(){
               ...
               var id = note.properties.id;
               var date_created = note.properties.date_created;
               var status = note.properties.status;
               if (status == "closed")
                status = 1;
               else
                status = 0;
               var comments = note.properties.comments;

               putNoteAJAXMarker(id, lon, lat, comments, status);
    ...
    makeRequestUrl : function(url)
    {
        return this.options.serverURL + "notes.json?" + url + "&nocache="+(new Date()).getTime();
    },

	createMarker: function(id)
	{
		var bug = putNoteAJAXMarker.bugs[id];
	...
	setPopupContent: function(id)
	{
		...
		var rawbug = putNoteAJAXMarker.bugs[id];
		...
		for (i=0; i<rawbug[1].length; i++)
		{
			...
			td.textContent = rawbug[5][i] + ': ' + L.i18n(rawbug[1][i].action);
...
//		var nickname = form.osbnickname.value || this.options.username;
	...
	closeBug: function(form) {
        $.ajax({
           type: "POST",
           data: ({text: form.osbcomment.value}),
           url:this.options.serverURL + "/notes/" + encodeURIComponent(form.osbid.value) + "/close",
        error: function () {
            alert('Error close note');}
        });
	},
...
//		form.osbnickname.value = this.options.username;
	...
	addBug: function(e) {
		...
		newContent.innerHTML += '<h3>'+L.i18n("New bug")+'</h3>';
...
L.OpenStreetNotes.setCSS = function() {
	if(L.OpenStreetNotes.setCSS.done)
		return;
	else
		L.OpenStreetNotes.setCSS.done = true;
...
function putNoteAJAXMarker(id, lon, lat, comments, closed)
{
...
//		isplit = comments[i].lastIndexOf("[");
			comments_only[i] = comments[i].text;
//			info = comments[i].substr(isplit+1);
            if (comments[i].user)
            {
			    nickname[i] = comments[i].user;
            }
            else
            {
                nickname[i] = "Анонимно";
            }
			datetime[i] = comments[i].date;
	}

	putNoteAJAXMarker.bugs[id] = [
	...
	for (i=0; i<putNoteAJAXMarker.layers.length; i++)
		putNoteAJAXMarker.layers[i].createMarker(id);
}

putNoteAJAXMarker.layers = [ ];
putNoteAJAXMarker.bugs = { };
